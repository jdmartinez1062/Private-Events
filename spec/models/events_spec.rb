...
RSpec.describe Event, type: :model do
  ...
  describe 'create event' do
    it 'Event creation ' do
      user.created_events.create(date: Time.now, location: "#{user.name}'s house")
  ...
  describe 'create event' do
    ...
    it 'Failed event creation due to empty date' do
      user.created_events.create(date: '', location: "#{user.name}'s house")
      expect(Event.count).to eql(0)
    end
    it 'Failed event creation due to empty location' do
      user.created_events.create(date: Time.now, location: '')
      expect(Event.count).to eql(0)
    end
    it 'Failed event creation due to empty attributes' do
      user.created_events.create(date: '', location: '')
      expect(Event.count).to eql(0)
    end
  end
end
